        ...
        run: |
          if [ "${{ github.event_name }}" = "workflow_dispatch" ]; then
            RUN_ID=$(gh run list \
              ...
              --jq '.[0].databaseId // ""')
            if [ -z "$RUN_ID" ]; then
              echo "No successful {{placeholder:site.rebuild.source.workflow}} run found on {{placeholder:publish.branch.gate}}" >&2
              exit 1
            fi
            echo "source-run-id=$RUN_ID" >> $GITHUB_OUTPUT
          else
            echo "source-run-id=${{ inputs['source-run-id'] }}" >> $GITHUB_OUTPUT
          fi
      ...
      - name: Aggregate optimized media
        run: |
          ...
          if [ -n "$DIRS" ]; then
            ...
            mkdir -p artifacts/optimized-summary/
          fi
