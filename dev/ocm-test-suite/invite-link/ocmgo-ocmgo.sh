...
# -----------------------------------------------------------------------------------
# Script to Test OCM-Go to OCM-Go OCM invite-link flow tests.
...
#   such as OCM-Go, using Cypress, and Docker containers.
...
main() {
    ...
    # No meshdir needed: OCM-Go handles invitations natively via its built-in invite API.

    if [ "${SCRIPT_MODE}" = "dev" ]; then
        run_dev \
            "https://ocm-go1.docker (username: marie, password: radioactivity)" \
            "https://ocm-go2.docker (username: einstein, password: relativity)"
    else
