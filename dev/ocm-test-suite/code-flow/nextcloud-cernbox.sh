...
# -----------------------------------------------------------------------------------
# Script to Test Nextcloud (sender) to CERNBox v2 (recipient) OCM code-flow tests.
# -----------------------------------------------------------------------------------
...
main() {
    ...
    local cernbox_revad_tag=sta-ocm-m6-development
    ...
    # Reva and Nextcloud-side changes land on their upstream default branches.

    create_nextcloud_code_flow 1 "marie" "radioactivity" "${nextcloud_image}" "${nextcloud_tag}"
