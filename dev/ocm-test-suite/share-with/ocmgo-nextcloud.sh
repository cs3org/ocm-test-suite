...
# -----------------------------------------------------------------------------------
# Script to Test OCM-Go to Nextcloud OCM share-with flow tests.
...
#   such as OCM-Go and Nextcloud, using Cypress, and Docker containers.
#   It supports both development and CI environments, with optional browser support.

# Usage:
#   ./ocmgo-nextcloud.sh [EFSS_PLATFORM_1_VERSION] [EFSS_PLATFORM_2_VERSION] [SCRIPT_MODE] [BROWSER_PLATFORM]

# Arguments:
#   EFSS_PLATFORM_1_VERSION : Version of the first EFSS platform (default: "v1.0.0-local").
...
# Example:
#   ./ocmgo-nextcloud.sh v1.0.0-local v27.1.11 ci electron
...
main() {
    ...
    if [ "${SCRIPT_MODE}" = "dev" ]; then
        run_dev \
            "https://ocm-go1.docker (username: marie, password: radioactivity)" \
