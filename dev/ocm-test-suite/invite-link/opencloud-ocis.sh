...
# -----------------------------------------------------------------------------------
# Script to Test Opencloud to oCIS OCM invite link flow tests.
...
#   specifically Opencloud and oCIS, using ScienceMesh integration and tools like Reva, Cypress, and Docker containers.
#   It supports both development and CI environments, with optional browser support.
# Usage:
#   ./opencloud-ocis.sh [EFSS_PLATFORM_1_VERSION] [EFSS_PLATFORM_2_VERSION] [SCRIPT_MODE] [BROWSER_PLATFORM]
# Arguments:
...
# Example:
#   ./opencloud-ocis.sh v2.3.0 v5.0.9 ci electron
# -----------------------------------------------------------------------------------
...
main() {
    ...
    prepare_opencloud_environment "opencloud1.docker,opencloud1.docker,dav/" "ocis1.docker,ocis1.docker,dav/"

    # Configure OCM providers for oCIS
    prepare_ocis_environment "opencloud1.docker,opencloud1.docker,dav/" "ocis1.docker,ocis1.docker,dav/"
    ...
    if [ "${SCRIPT_MODE}" = "dev" ]; then
        run_dev \
          ...
          "https://ocis1.docker (username: michiel, password: dejong)"
    else
