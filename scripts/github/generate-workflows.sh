...
main() {
    ...
    # Only delete files generated by this script – keep the engine, orchestrator, pages etc.
    find "$workflows" -maxdepth 1 -type f -regextype posix-extended \
        -regex ".*/(login|share-link|share-with|invite-link|wayf|code-flow).*\.yml" -delete
