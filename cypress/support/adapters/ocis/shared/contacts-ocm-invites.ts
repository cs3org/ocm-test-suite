...
// oCIS OCM / ScienceMesh invite helpers.
...
// OCM app menu IDs across oCIS versions. open-cloud-mesh is v8+; older
...
export function resolveOcmReceiverBaseUrl(): string {
...
function openOcmViaAppSwitcher(): void {
  ...
  // After the switcher expands, click the first matching OCM app link.
...
export function openOcmApp(): void {
  cy.viewport(1280, 720);
  cy.visit("/");
  cy.get("#web-content", { timeout: ocmActionTimeoutMs }).should("be.visible");
  openOcmViaAppSwitcher();

  // Verify we reached the OCM app by checking any of the known section IDs.
  cy.get(
...
// assertOcmConnectionExists decodes and verifies the connection.
export function encodeOcmAcceptedContactUrl(senderDomain: string): string {
  return `ocm-contact://${senderDomain}`;
}

export function decodeOcmAcceptedContactUrl(acceptedContactUrl: string): string {
...
export function createOcmInviteTokenBase64(note: string): Cypress.Chainable<string> {
  openOcmApp();
  ...
  // cy.stub is sinon-backed and restored after each test; it does not affect
  // other helpers or globally suppress uncaught exceptions.
  cy.window().then((win) => {
    if (
  ...
  // In the dialog, optionally fill a note then click "Generate".
  ...
  // Primary: .oc-modal-body-actions-confirm (modal confirm class).
  // Fallback: text-based for environments that do not expose that class.
  ...
  // Wait for the invite row to appear then read the base64 code.
  cy.get("#sciencemesh-invite table tbody tr", { timeout: ocmApiTimeoutMs })
    .first()
    .should("be.visible");

  // Read the sender host from the AUT location once; needed in the fallback
  // path below to compose base64(token@host).
...
export function acceptOcmInviteToken(
  inviteToken: string,
  ...
  // Find the token input via the "Enter invite token" label and type token.
  ...
  // The accept button is a parent of the "Accept invitation" span.
  cy.get("#sciencemesh-accept-invites")
    .find("span")
  ...
  // Wait for at least one connection row to confirm acceptance.
  ...
  return cy.wrap(encodeOcmAcceptedContactUrl(senderDomain), { log: false });
}

export function assertOcmConnectionExists(params: {
  acceptedContactUrl: string;
}): void {
  const senderDomain = decodeOcmAcceptedContactUrl(params.acceptedContactUrl);

  // After acceptOcmInviteToken we stay in the OCM app; confirm connections.
