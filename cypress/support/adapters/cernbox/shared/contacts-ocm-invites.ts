...
// CERNBox OCM / ScienceMesh invite helpers (cernbox-web selectors).
...
export function resolveCernboxOcmReceiverBaseUrl(): string {
...
export function openCernboxOcmApp(): void {
...
export function encodeCernboxAcceptedContactUrl(senderDomain: string): string {
  return `ocm-contact://${senderDomain}`;
}

export function decodeCernboxAcceptedContactUrl(
...
export function createCernboxInviteTokenBase64(
  note: string,
  ...
  cy.window().then((win) => {
    if (
      ...
      !(win.navigator.clipboard.writeText as unknown as { isSinonProxy?: boolean })
        .isSinonProxy
    ) {
...
export function assertCernboxConnectionExists(params: {
  acceptedContactUrl: string;
}): void {
  const senderDomain = decodeCernboxAcceptedContactUrl(params.acceptedContactUrl);
